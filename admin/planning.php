...
<!-- Contenu -->
<!-- L'affichage doit se présenter sous la forme d'un premier calendirer avec les mois, semaines(jours) et l'année correspondante. 
Quand on rentre l'ID d'un salarié, son planning se génère automatiquement puis on peut accéder au planning détailler (second planning) qui renseigne les horraires de travaille chaque jours-->
...
<div class="container">
	...
	<?php
		if(isset($_POST['requete']) && $_POST['requete'] != NULL) // on vérifie d'abord l'existence du POST et aussi si la requete n'est pas vide.
		{
			...
			$query = mysql_query("SELECT * FROM salarie WHERE nomSalarie LIKE '%$requete%' ORDER BY idSalarie DESC") or die (mysql_error()); // la requête, que vous devez maintenant comprendre ;)
		...
		if($nb_resultats != 0) // si le nombre de résultats est supérieur à 0, on continue
		{
			...
			?>
			 dans notre base de données. Voici les Salariés que nous avons trouvées :<br/>
			<br/>
			<?php
			...
			{
			?>
			<a href="planning.php?id=<?php echo $donnees['idSalarie']; ?>"><?php echo $donnees['nomSalarie']; ?></a><br/>
			<?php
			} // fin de la boucle
			?><br/>
			<br/>
